...
let AiService = class AiService {
    ...
    async draftFormalDocument(baseDraft, metadata) {
        if (!process.env.OPENAI_API_KEY) {
            return baseDraft;
        }
        try {
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'Eres un asistente de redacción formal para comunicaciones institucionales. ' +
        ...
        try {
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'Eres un asistente de búsqueda documental institucional en español. ' +
